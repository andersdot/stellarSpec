...
def plot_sample(x, y, samplex, sampley, xdgmm, xlabel='x', ylabel='y', xerr=None, yerr=None, ylim=(6, -6), xlim=(0.5, 1.5), errSubsample=1.2e6, thresholdScatter=0.1, binsScatter=200, c='black',  norm=None, cmap=None, contourColor='k', posterior=False, ind=None, plot_contours=True), sdss5=False:
    ...
    im = corner.hist2d(x, y, ax=ax1, levels=levels, bins=200, no_fill_contours=True, plot_density=False, color=contourColor, plot_contours=plot_contours)
    ...
    if norm is not None:
        ...
        cb.set_clim(-7, 2)
    """
    #pp1 = PdfPages('plot_sample.data.pdf')
    #pp2 = PdfPages('plot_sample.prior.pdf')

    #pp1.savefig(figData)
    #pp2.savefig(figPrior)
