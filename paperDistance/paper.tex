...
% - make a ``why does this work'' section
...
\begin{equation}
\label{eq:bayes}
P(d_{true}|\varpi, \sigma^2_{\varpi}, \textbf{I}) = \frac{P(\varpi |d_{true} \sigma^2_{\varpi}, \textbf{I}) P(d)}{\textbf{Z}},
\end{equation}
...
True values:
  C - true color,
  M - true absolute magnitude
  Y = [10.^0.2(M+2), C]
data:
    (J-K)_c
    \varpi
    J_c
    y = [\varpi10^0.2J_c, (J-K)_c]

  prior assumptions:
    \sigma_J, sigma_K, sigma_\varpi, Greene map
...
\label{eq:data}
\begin{aligned}
\mathrm{true vector} \;\;\;\; &\vec{Y} = [10^{0.2\,M + 2}, C] \\
\mathrm{data vector} \;\;\;\; &\vec{y} = [\varpi\,10^{0.2J_{c,n}}, (J- K)_{c,n}] \;  n \in \mathrm{stars} \\
...
\mathrm{assumptions} \;\;\;\; &I = \left\{\left\{\sigma^2_n\right\}_{n=1}^N, \mathrm{assumptions}\right\}
\end{aligned}
\end{equation}
...
\alpha =& \left\{\mathrm{amp}_k, \mu_k, V_k\right\}_{k=1}^K \; k \in \mathrm{Gaussians} \\
       ...
       =& \argmax_{\alpha} \Pi_n \int P(\vec{y}_n|\vec{Y}_n,I)P(\vec{Y}|\alpha)d\vec{Y}
\end{aligned}
\end{equation}

\begin{equation}
\label{eq:posterior}
P(Y|y, \alpha, I) = 1/Z p(y| Y,I)p(Y|\alpha, I)
\end{equation}

\begin{equation}
\label{eq:parallaxPost}
P(\vec{Y}_1|\vec{y}, \alpha, I) => p(\varpi_{true}|y, \alpha, I) \, f(\varpi_{true})
\[
