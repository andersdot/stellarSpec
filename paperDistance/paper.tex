...
Our true, latent values are C, the true J-K color, and M the true absolute magnitude, which we encapsulate in the true vector $\vec{Y}$. The observed data is the dust corrected J-K color $(J-K)_n$, the Gaia parallax $\varpi_n$, and the dust corrected J band magnitude $J_c$, which we encapsulate in the data vector.  Our prior assumptions $I$ are the uncertainties in each of these measurements, as well as the Greene dust map
...
\mathrm{with \; absolute \; magnitude} \;\;\;\; &M^c_{J,n} = J_n + 5\log_{10}\varpi_n - Q_JE(B-V)_n \\
...
where $E(B-V)$ is the output of the Schlafly dust model, and $Q$ is the correction factor for that band assuming some reddening law.

Working with these variables, we then have a likelihood for the data, both the Gaia parallaxes and 2mass photometry of

\begin{equation}
\label{eq:fulllike}
P(\vec{y}|\vec{Y}) = \mathcal{N}(\vec{y}|\vec{Y}, \sigma_{\vec{y}})
\end{equation}

To generate our prior, we fit the data in the CMD space using XD which deconvolves the data with its noise, which are heterogeneous measurement uncertainties. It generates the denoised, underlying distribution from which the noisey data were drawn from. It represents the distribution as a mixture of gaussians which maximizes the marginalized likelihood of the noisy data, marginalized over the true, latent variables.
...
\alpha =& \left\{\mathrm{amp}_k, \mu_k, V_k\right\}_{k=1}^K \; k \in \mathrm{Gaussians} \\
       =& \argmax_{\alpha} \, \Pi_n \, P(\vec{y}_n|\alpha) \\
       =& \argmax_{\alpha} \, \Pi_n \, \int P(\vec{y}_n|\vec{Y}_n) \, P(\vec{Y}|\alpha)d\vec{Y} \\
\end{split}
\end{equation}

\begin{equation}
\label{eq:posterior}
P(\vec{Y}|\vec{y}, \alpha) = \frac{1}{Z} \, P(\vec{y}|\vec{Y}) \, P(\vec{Y}|\alpha)
\end{equation}

\begin{equation}
\label{eq:parallaxPost}
\begin{split}
P(\vec{Y}_0|\vec{y}, \alpha) => P(\varpi_{true}|y, \alpha) \, f(\varpi_{true}) \\
\[
