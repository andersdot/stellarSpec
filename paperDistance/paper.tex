...
% # To-do list
% - We need a standard name for the \tgas--\tmass\ Intersection.
...
The data are a transformation of the dust corrected absolute magnitude, and the dust corrected colors.

\begin{equation}
\label{eq:data}
\begin{aligned}
\mathrm{data vector} \;\;\;\; &\vec{y} = [10^{0.2M_{J,n}}, J_n - K_n] \;  n \in \mathrm{stars} \\
\mathrm{with \; absolute \; magnitude} \;\;\;\; &M_{J,n} = J_n - DM_n - Q_JE(B-V)_n \\
& DM_n = -5\mathrm{log_{10}}\frac{1}{\varpi} \\
\mathrm{and \; color} \;\;\;\; &J_n - K_n = J_n - K_n - Q_{JK}E(B-V)_n

\end{aligned}
\end{equation}

We fit the data using XD which maximizes the likelihood
