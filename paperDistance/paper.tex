...
Think about presentation, prioritizing/highlighting prior but logically building it from an full inference perspective and needing a prior, then dive into prior stuff. It's confusing right now jumping between inference and prior building !! 
...
Supplementary to this trigonometric measure of distance, the photometry of stars can improve the distance inference.
...
The tight correlation of the \cmd\ benefits our inference because the absolute magnitude of a star contains distance information.
...
Our prior assumptions $I$ are the uncertainties in each of these measurements, as well as the Greene dust map
...
If life were simple, our inference would be a 1D problem using Bayes theorem looking something like:
\begin{equation}
...
Here we have assumed that the parallas uncertainty dominates over the $J$ band uncertainty, so we take $J_n^C$ as a point estimate and only propagate the parallax uncertainty. This is to keep our uncertainties Gaussian so we can use \xd\ generate our prior below.
This is the probability of the data vector given the true vector, but we
...
We used cross validation to show that the likelihood of the data continued to increase from 16 Gaussians to 2048 Gaussians. We chose to do our inference using 128 Gaussians, but increasing the number of Gaussians was also a good check to see what features in the CMD appeared to be real, and which ones smoothed out with increasing numbers of Gaussians. The most obvious structures that persisted to our maximum number of Gaussians are:
\begin{itemize}
\item Main Sequence
\item Binary Sequence
\item Possible metallicity variation in MS lifetime
\item Red clump
\item Red bump
\item Red Giant Branch
\item Asymptotic Giant Branch
...
Some discussion of the inconsistency of how we treat dust within the probabilistic framework.
