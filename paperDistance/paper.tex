...
However, there is multi-band photometry for a large fractino of the \gaia\ \tgas\ catalog from imaging surveys;
this imaging is incredibly informative about stellar distances.
Here we use color information on $1.2$ million \tgas\ stars from \tmass\ to build a noise-deconvolved empirical prior distribution for stars in color--magnitude space.
...
Our posterior parallax estimates are more precise than the \tgas\ catalog entries by a median factor of [XXX]; and more precise than previous examples of Bayesian distance estimates by a factor of [YYY].
The precision can be attributed to the statistics concept of shrinkage.
We independently validate our distances by looking at members of Milky Way star clusters; for example, M67 is not visible as a cluster in the \tgas\ parallax estimates, but appears clearly in our posterior parallax estimates.
...
stars, and calibrate to spectrophotometric models.
...
To ease understanding, both for ourselves and the reader, here we will demonstrate this method with a simpler toy model.
Instead of inferring more precise parallax pdfs using the 2D distribution of the \cmd\ as our prior, for this simpler model we will infer more precise pdfs of the value $Y$ using a 2D distribution that is a 1D Gaussian in the y-direction, with a running mean $\mu = mx + b$ and some thickness $t$, as our prior.
...
First we determine the underlying distribution that the noisy data $y_n$ are drawn from. The $1\sigma$ contours of this noise deconvolved distribution are shown as the blue lines in the lower left panel of Figure \ref{fig:toy}.
This noise deconvolved distribution becomes our prior.
We then use this pior to infer a posterior belief about the true values, $p(y_{\true,n} \given y_n, \tilde{\sigma}_n^2)$, whose expectation value and $1\sigma$ uncertainty is shown as the blue points in the lower panels of Figure \ref{fig:toy}.
