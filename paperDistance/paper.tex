...
\caption{ {\bf Why does this work}: An example of generating posterior beliefs of noisy data by using the denoised distribution of data as a prior. The left panel is 1024 realizations of noisy data drawn from the gaussian distribution represented by the $1\sigma$ red lines shown in the center panel. The black points are 5 randomly chosen data to highlight the method. The blue lines in the center panel represent the MML best estimate for the underlying distribution, which denoises the data. The blue points in the center panel represent the posterior belief of the true values of the noisy black points, and the red points represent the actual true values. The right panel shows the posterior belief of the true value for all the points. They are colored by the measurement variance showing that data with larger measured variance are more influenced by the prior and lie towards the center of the MML distribution, or prior. Where as more certain measurements tend to lie closer to their measured value. The blue points are better than the black for doing stuff.}
\label{fig:line}
\end{figure}

In this paper we use the denoised, observed \cmd\ as a prior to infer more precise parallax pdfs from \gaia\ measurements.
Here we will demonstrate this method with a simple toy mode. Instead of using the \cmd\ as our prior to get more precise measurements of parallax, we use a straight line $y = mx + b$ with some gaussian thickness $t$ as our prior. We use this prior to infer some posterior belief over $y_{\true,n}$ from noisy $y_n$ data.
Here the thickness of the true distribution mimics the true thickness of the \cmd\ due to age, mass and metallicity.
To set up the problem we have the true underlying distribution $y_{\true, n} = m_{\true}\,x_n + b_{\true} + \Delta_n$ where $\Delta_n$ is drawn from $\mathcal{N}(\Delta_n \given 0, t^2)$, where $\mathcal{N}$ represents a Gaussian function, here with mean $0$ and variance $t^2$.
...
From this we generate our noisy data $y_n = y_{\true,n} + \epsilon$ where $\epsilon$ is drawn from $\mathcal{N}(\epsilon \given 0, \sigma_n^2)$.
...
We then use this pior to infer a posterior belief about the true values, $p(y_{\true,n} \given y_n, \tilde{\sigma}_n^2)$.
...
We denoise these points using marginalized maximum likelihood estimation (MML), which marginalizes over the true data values (we never need to know them) and maximizes the likelihood of the data. Here our likelihood for each individual datum is $p(y_n \given m, b, t^2, \sigma_n^2) = \mathcal{N}(y_n \given m\,x_n + b, t^2 + \sigma_n^2)$ and the likelihood of the full data set is $\prod_n\, p(y_n \given m, b, t^2, \sigma_n^2)$. Maximizing this likelihood is the same as minimizing the $\chi^2 = \sum_n \frac{[y_n - [m\,x_n + b]]^2}{[\sigma_n^2 + t_n^2]} + \sum_n\,ln(2\pi\,[\sigma_n^2 + t^2])$. This MML gives us the best fit parameters $\hat{m}$, $\hat{b}$, and $\hat{t}$. In Figure \ref{fig:line}, center panel, the true functions $y_{\true} = m_{\true}\,x + b_{\true} \pm t_{\true}$ are shown as the red lines, and the best fit MML functions $\hat{y}_{\true} = \hat{m}\, x + \hat{b} \pm \hat{t}$ are shown as the blue lines. Now that we have an estimate of the true underlying distribution $\hat{y}_{\true}$, we use this as a prior to infer a posterior belief of the true values for each datum $p(y_{\true,n} \given y_n, \tilde{\sigma}_n^2)$. Here is a good point to step back and acknowledge that our method is odd/strange/different/new (which maybe you're also currently thinking but can't quite express why yet). We are using the set of data to determine the denoised distribution, and then using that denoised distribution to make inferences on the data. We are technically using the data twice! If we were going to be fully, technically correct, we should denoise the distribution, but leave one datum out; the one datum we want to infer the true y value for\footnote{DWH: Remember what this brain dump is for}. Leaving one out, and then estimating the prior and then inferring the posterior for each 1024 points, let alone over a million points is way too expensive. Luckily with large data sets, even at 1024, 1 datum is $\sim0.1\%$ of your dataset and therefore leaving that point out will not change the MML parameters of your denoised distribution, and with the \gaia\ dataset of over a million stars, we can safely step into the Empirical Bayes regime.
...
Since everything is Gaussian, this can be done analytically by multiplying Gaussians. The blue points in Figure \ref{fig:line}, center panel, represent the posterior belief of the true value $p(y_{\true,n} \given y_n, \tilde{\sigma}_n)$. The black points represent the original, noisy data $y_n \pm \sigma_n$, the same as those highlighted in panel A, and the red points are the true values $y_{\true, n}$. Finally, Figure \ref{fig:line}, right panel, shows the full set of posterior beliefs for each $y_n$. The points are colored by their measurement variance $\sigma_n^2$ to show that noisier measurements are more influenced by the prior and get pulled closer to the center of the MML distribution, our prior, while more certain measurements remain closer to their measured values $y_n$.

LA: Conclusions and stuff about why this stuff is good
